...
class mlperf_logger(object):
    ...
    def __init__(self, filename, benchmark, organization, machine_scale):
        ...
        self.comm_rank = comm.get_data_parallel_rank()
        self.comm_local_rank = comm.get_local_rank()
        self.comm_size = comm.get_data_parallel_size()
        self.comm_is_root = (self.comm_rank == comm.get_data_parallel_root())
        self.constants = constants

        # create logging dir if it does not exist
        logdir = os.path.dirname(filename)
        if self.comm_rank == comm.get_data_parallel_root():
        ...
        self.log_event(key = constants.SUBMISSION_PLATFORM,
                       value = f'{machine_scale}xNVIDIA DGX A100')
    ...
    def barrier(self):
        ...
        if dist.is_available() and dist.is_initialized():
            dist.barrier(device_ids = [self.comm_local_rank], group=comm.get_data_parallel_group())
