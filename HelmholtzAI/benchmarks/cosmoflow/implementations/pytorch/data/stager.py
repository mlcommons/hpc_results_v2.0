...
import libCosmoflowExt as cf_ext
...
def numpy_integrity_check(src_dir, dst_dir, files):
    ...
    for fname in files:
        ...
        checklist.append({"file": fname, "equal": compare,
...
def load_file(filename, block_size=512, file_size=0):
    token = cf_ext.read_file(filename, block_size, 0)
    return filename, token, len(token)


def save_file(ofname, fdata):
    cf_ext.write_file(ofname, fdata, 4096, True)
    return len(fdata)
...
class FileStager(object):
    def _load_batch_parallel(self, source_directory, files, num_shards, shard_id, filesize=None, blocksize=None):
        ...
        for filename in files_load:
            ...
            queue.append(self.executor.submit(
                load_file, fname, blocksize, filesize))
        nvtx.range_pop()
    ...
    def _finalize_save_local(self, save_queue):
        ...
        wbytes = 0
        cf.wait(save_queue, return_when=cf.ALL_COMPLETED)
        nvtx.range_pop()
    ...
    def _distribute_save_batch_parallel(self, comm, load_queue, target_directory):
        ...
        for load_future in cf.as_completed(load_queue):
            fname, fdata, fbytes = load_future.result()
            ...
            if comm is not None:
                ...
                for fname, fdata in zip(fname_all, fdata_all):
                    ofn = os.path.join(
                        target_directory, os.path.basename(fname))

                    #save_file(ofn, fdata, self.async_handler)
                    save_queue.put(self.executor.submit(
                        save_file, ofn, fdata))
                    rbytes += len(fdata)
            else:
                ofn = os.path.join(target_directory, os.path.basename(fname))
                #save_file(ofn, fdata, self.async_handler)
                save_queue.append(self.executor.submit(
                    save_file, ofn, fdata))
                rbytes += len(fdata)
    ...
    def _stage_batch_data(self, shardinfo,
        ...
        save_queue, rbytes = self._distribute_save_batch_parallel(
            comm, load_queue, target_directory)
    ...
    def __init__(self,
        ...
        self.stage_comm = self.global_comm.Split(
            color=self.irank, key=self.instance_id)
        ...
        self.local_comm = self.instance_comm.Split(
            color=(self.irank // self.lsize), key=self.lrank)
    ...
    def _prepare_instance_stage(self, files, target_directory):
        if (self.stage_mode == "node") and self.full_dataset_per_node:
            ...
            files_shard = get_shard(
                files, self.isize, self.irank, cycle_dist=self.lsize)
        ...
        # create tags
        # Original tag from Deepcam.  Modified for Cosmoflow
        # tag = os.path.basename(files[0]).split("-")[0]
        #
        tag = os.path.splitext(files[0].split("-")[-1])[0]
    ...
    def prepare(self, data_dir_prefix, stage_dir_prefix, stage_filter_list):
        target_directory = os.path.join(stage_dir_prefix, "")
        ...
        for stage_filter in stage_filter_list:
            ...
            stage_source_directory = os.path.join(
                data_dir_prefix, os.path.dirname(stage_filter))
            stage_target_directory = os.path.join(
                target_directory, os.path.dirname(stage_filter))
            ...
            if self.grank == 0:
                allfiles = sorted([os.path.basename(x) for x in glob(
                    os.path.join(stage_source_directory, os.path.basename(stage_filter)))])
            else:
                allfiles = None
            ...
            list_file = self._prepare_instance_stage(
    ...
    def _stage_instance_data(self, stage_filter):
        ...
        #print(f"STAGER DEBUG: {files_shard}")
        ...
        for stage_filter in self.file_stats.keys():
            ...
            if self.verify:
                ...
                if self.lrank == 0:
                    ...
                    staged_files = glob(os.path.join(
                        stage_target_directory, os.path.basename(stage_filter)))
                else:
                    staged_files = []
                ...
                if not self.full_dataset_per_node:
                    ...
                    staged_files_full = self.instance_comm.allgather(
                        staged_files)
                    staged_files_full = set(
                        itertools.chain(*staged_files_full))
                else:
                ...
                if self.irank == 0:
                    ...
                    global_files_full = glob(os.path.join(
                        stage_source_directory, os.path.basename(stage_filter)))
                global_files_full = self.instance_comm.bcast(
                    global_files_full, 0)
                global_num_files = len(global_files_full)

                # strip off the directory
                checkfiles1 = sorted([os.path.basename(x)
                                     for x in staged_files_full])
                checkfiles2 = sorted([os.path.basename(x)
                                     for x in global_files_full])
                ...
                if self.extended_verify:
                    if self.lrank == 0:
                        ...
                        if issue:
                            print(
                                f"Instance {self.instance_id}, local rank {self.irank}: Verification Error. Results:", checks, flush=True)
                        else:
                            print(
                                f"Instance {self.instance_id}, local rank {self.irank}: Verification OK", flush=True)
                    self.instance_comm.Barrier()

                if self.irank == 0:
                    print(
                        f'Staged data for {stage_filter}: {staged_num_files}, expected: {global_num_files}', flush=True)
                nvtx.range_pop()
