...
if [ "$TRAINING_SYSTEM" == "juwelsbooster" ] || [ "$TRAINING_SYSTEM" == "booster" ] || [ "$TRAINING_SYSTEM" == "jb" ]
  then
    export OUTPUT_ROOT="${PWD}/../../../../../results/juwelsbooster_gpu_n${ngpus}_pytorch1.11/${STRONGWEAK}/cosmoflow/"
...
#      --gpu-freq="high"
  ...
  then
    ...
    export OUTPUT_ROOT="${base_dir}results/horeka_gpu_n${ngpus}_pytorch1.11/${STRONGWEAK}/cosmoflow/"
    export OUTPUT_DIR="${OUTPUT_ROOT}"
    echo "${OUTPUT_ROOT}"
