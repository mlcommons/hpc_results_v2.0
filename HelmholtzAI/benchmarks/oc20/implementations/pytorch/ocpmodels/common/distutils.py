...
def setup(config):
    ...
    # juwelsbooster and horeka require different init methods, use the environment variable PROCESS_GROUP_INIT_METHOD to
    # switch between these variants. Use the default method if PROCESS_GROUP_INIT_METHOD is unset or != BOOSTER, use the
    # alternative init method for juwelsbooster if PROCESS_GROUP_INIT_METHOD == BOOSTER
    if os.environ.get("PROCESS_GROUP_INIT_METHOD") == "BOOSTER":  # alternative init method required for juwelsbooster
        hostname = gethostname().split('.')[0]+"i."+gethostname().split('.')[1]
        init_method = f"tcp://{per_instance_comm.allgather(hostname)[0]}:12345"
    else:  # default init method, used on horeka
