...
def get_num_samples(config, mode):
    # on jb: lmdb cannot open files on cscratch so when using cscratch as data source set DATA_SRC_SCRATCH to an
    # alternative data source on scratch
    data_src = os.getenv('DATA_SRC_SCRATCH', config["data"])
    srcdir = Path(data_src, config[f"{mode}_dataset"])
