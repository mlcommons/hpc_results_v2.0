...
def copy_dataset(config):
    if config["data"] == config["data_target"]:  # don't copy if src and target destination are the same
        return

    if config["local_rank"] == 0:  # only one rank per node does the copying
        data_source, data_target = config["data"], config["data_target"]
        number_of_copy_threads = config["jobs"]

        # define source and target directories for training and validation
        train_suffix, val_suffix = "oc20_data/s2ef/2M/train", "oc20_data/s2ef/all/val_id"
        train_source, val_source = f"{data_source}/{train_suffix}", f"{data_source}/{val_suffix}"
        train_destination, val_destination = f"{data_target}/{train_suffix}", f"{data_target}/{val_suffix}"

        # ensure the target directories exist
        os.makedirs(train_destination, exist_ok=True)
        os.makedirs(val_destination, exist_ok=True)

        # get all training/validation files in the corresponding source directories
        training_files, validation_files = glob(f"{train_source}/*"), glob(f"{val_source}/*")

        # define copy functions copying to the train/val target directory using shutil.copy
        copy_training_data = partial(shutil.copy, dst=train_destination)
        copy_validation_data = partial(shutil.copy, dst=val_destination)

        # copy the data using a thread pool
        with ThreadPool(number_of_copy_threads) as pool:
            pool.map(copy_training_data, training_files)
        with ThreadPool(number_of_copy_threads) as pool:
            pool.map(copy_validation_data, validation_files)

    # synchronize; after this the data is available to all ranks on this node
...
class MLPerfForcesTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        if distutils.is_master():
            ...
            mllogger.event(key=mllog.constants.SUBMISSION_ORG, value="HelmholtzAI")
            ...
            number_of_nodes = int(os.environ.get("SLURM_NNODES", 1))
            mllogger.event(key=mllog.constants.SUBMISSION_PLATFORM, value=f"{number_of_nodes}xNVIDIA DGX A100")
